...
*** LinearSVC
    ...
    parameter "C" controller soft margin classification balance, smaller C leads wider street but more margin violations
    parameter "tol" controlle tolerance in classification
    computation complexity is O(m * n)
*** LinearSVR
    parameter "epsilon" controller street width in regression
*** SVR
    parameter "epsilon" controller street width in regression
    parameter "C" controller regularization, smaller C leads more regularization
** linear_model
*** LinearRegression
*** SGDRegressor

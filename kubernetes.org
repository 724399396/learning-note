...
* Best practice
** Centralize log
   use KEF(Kibana ElasticSearch FluentD) to as centralized log solution
** minikube
   use minikube mount to mount your local files into minikube vm
   use minikube inside docker daemon build docker, avoid network fetch
   docker save <image> | (eval $(minikube docker-env) && docker load), load local image to minikube
** CD
   [[https://github.com/box/kube-applier]] deploy from vcs
   [[http://fabric8.io]] full CI/CD pipeline for DevOps
** json/yaml
   use ksonnet/jsonnet to create manifest file
* Extension Kubernetes 
  ...
  CustomResourceDefinition.spec.group/CustomResourceDefinition.spec.version is correspond to apiVersion 
  ...
  from kubernetes 1.8, use CustomResourceValidation feature validate custom object post json/yaml file

  from kubernetes 1.8, api server can back end with batch of api server aggerage, you can define you custom api server 

  service Catalog, is a catelog of services, user can through catelog create service without deal with pods, serviecs, configmaps and other

  Service Catalog:
  - CusterServiceBroker, which describes an (external) system that can provision
services
  - ClusterServiceClass, which describes a type of service that can be provisioned
  - ServiceInstance, which is one instance of a service that has been provisioned
  - ServiceBinding, which represents a binding between a set of clients (pods)
and a ServiceInstance
  ...
  - ImageStream
  - Routes, pre Ingress Controller
  - others

  Deis workflow is a PaaS, workflow also provide a source to deployment solution, need helm CLI to work on deis workflow 

  can deploy Deis workflow to any existing Kubernetes cluster, when run workflow, it create a set of Service and ReplicationController

  Helm is a package manager for kubernetes, can work without workflow
  
  Tiller server is a pod run on k8s, Helm application package named as charts 
